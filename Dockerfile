...
RUN chmod +x /bin/dumb-init; \
    ...
    yum install -y code-server-3.3.1-amd64.rpm golang delve bash git python36 java-11-openjdk nodejs make gcc libffi-devel python36-devel openssl-devel ncurses; \
    ...
    cd /lynx2.8.9rel.1 && ./configure && make && make install ;\
    yum clean all; \
    rm -rf /var/cache/yum; \
    mkdir /coder; \
    chgrp -R 0 /coder; \
